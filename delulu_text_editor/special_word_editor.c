/**
 * It will be used for the side project of delulu text editor.
 ...
 * Thinking of using it to do some special word editing in the delulu text editor. where when written a special word only then it
 * will be used to edit the word.
 * This program is a simple terminal input reader that captures characters in raw mode,
 * allowing for real-time input without line buffering or echoing. It reads characters one by one
 * and prints their ASCII values, distinguishing between control characters and regular characters.
 * The program continues to read input until the character 'p' is pressed, at which point 
 * it exits gracefully.
 */
